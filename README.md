...
　- ファイルサイズは2GB以下までしか扱えない　　
 - ファイルに対しては直接の操作ではなくインターフェース（`io.ReadWriteSeeker`）越しの読み書きしか行わないので`Flush`や`Close`などの処理等は呼び出し側のほうで行う必要がある  
 - データのサイズの変わる更新や削除を行うと使用できないゴミ領域が増えるが対処法は無い
